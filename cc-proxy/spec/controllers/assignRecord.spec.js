...
function nockAuth(isTeamLeader) {
	...
	const tagNock = nockRaisely()
		...
		.reply(200, {
			data: { uuid: 'caller-uuid', tags: [{ path: isTeamLeader ? 'team-leader' : 'facilitator' }] },
		});
}
...
function nockAssignment() {
	noteRequest('POST /users/assignments', 'POST', '/users/user-uuid/assignments');
}

function nockCheckAssignment(shouldFind) {
	noteRequest('GET /users/assignments', 'GET', '/users/caller-uuid/assignments/users/target-uuid', shouldFind ? 200 : 404);
}

function itAssignsUser(shouldCall = true) {
	it(`it ${shouldCall ? 'assigns' : 'does not assign'} the user`, () => {
		const key = 'POST /users/assignments';
		expect(!!requests[key], 'Assignment call was not made').to.eq(shouldCall);
		if (shouldCall) {
			expect(requests[key].body).to.deep.eq({ data: [_.pick(assignBody, ['recordType', 'recordUuid'])] });
		}
	});
}
...
function itCheckedAssignment() {
	it('checks assignment', () => {
		expect(!!requests['GET /users/assignments'], 'Assignment was not checked').to.be.true;
	});
}
