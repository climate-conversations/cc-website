(RaiselyComponents, React) => {
	...
	async function hasReflection(conversation, rsvps) {
		...
		const complete = facils
			...
			.filter((rsvp) =>
				reflections.find((r) => r.user.uuid === rsvp.uuid)
			);
	...
	function donationsReported(conversation) {
		return (
			get(conversation, 'private.cashReceived') === false ||
	...
	const checklist = [
		{
			...
			label: 'Review facilitators Reflection',
			href: '/conversations/:event/reflections/view',
			enable: hasReflection,
		},
		{
			id: 'donation-review',
			label: 'Review Cash Donations',
			href: '/conversations/:event/reconcile-donations',
			enable: donationsReported,
			done: donationsReconciled,
		},
		{
			id: 'review-stats',
			label: 'Review Conversation Outcomes',
			href: '/conversations/:event/outcomes',
			enable: atLeastOneGuest,
			done: conversationReviewed,
		},
	];
	const checklistHelp = {
		'review-reflection':
			'Check and see how your facilitator is feeling about the conversation and if you need to get in touch',
		'donation-review':
			'Help us keep good records of our cash donations by checking the report submitted',
		'review-stats':
			'See what the facilitator achieved in this conversation',
	};
	const checklistDisabled = {
		'review-reflection':
			'Facilitator has not completed their reflection yet',
	...
	return class TeamLeaderProcessingChecklist extends React.Component {
		...
		getCompletedSteps = (conversation) =>
			get(conversation, 'private.ftlCompletedSteps', '')
				.split(';')
				.sort();
			...
			await Promise.all(
				checklist.map(async (item) => {
					if (!completedSteps.includes(item.id)) {
						if (item.enable) {
							// eslint-disable-next-line no-param-reassign
							item.isEnabled = await item.enable(
								conversation,
								rsvps
							);
						}
						if (item.isEnabled || !item.enable)
							enabledSteps.push(item.id);
						if (item.done) {
							try {
								// eslint-disable-next-line no-param-reassign
								item.isDone =
									item.isEnabled &&
									(await item.done(conversation, rsvps));
								...
								console.error(
									`Checking done status failed for ${item.id}`,
									e
								);
								this.setState({
		...
		updateComplete = async (conversation, completedSteps) => {
			...
			const allDone = checklist.reduce(
				(done, i) => i.isDone && done,
				true
			);
			if (hasChanged) {
				...
				await getData(
					api.events.update({
						...
						data: {
							data: {
								private: { ftlCompletedSteps: stepsDoneNow },
							},
						},
					})
				);
		...
		async load() {
			try {
				...
				const eventPromise = Conversation.loadConversation({
					props: this.props,
					required: true,
					private: 1,
				});
				...
				const [conversation, rsvps] = await Promise.all([
					...
					Conversation.loadRsvps({
						props: this.props,
						type: ['facilitator', 'co-facilitator', 'guest'],
					}),
				]);

				const completedSteps = await this.checkCompletedSteps(
					conversation,
					rsvps
				);
		...
		render() {
			...
			return (
				<div className="conversation--checklist__wrapper">
					<h3>conversation details {name}</h3>
					<div className="conversation--checklist_subtitle">
						{displayDate}
					</div>
					...
					{isReviewed ? (
						<p>
							Greate work! Thanks for helping to keep our record
							keeping accurate
						</p>
					) : (
						''
