		...
		async load() {
			...
			try {
				...
				const [
					...
					// Load training rsvps
					getData(
						api.eventRsvps.getAll({
							query: {
								'event.eventType': 'Facilitator Training',
								'event.startAt': since,
								campaign: Conversation.getWebsiteUuid(),
								private: 1,
							},
						})
					),
				...
				const interestedFacilitators = new Set(
					postSurveys.map((s) => s.userUuid)
				);
				...
				const allTraineeRsvps = new Set(
					trainingRsvps.map((r) => r.userUuid)
				);
				const allTraineeCompletes = new Set(
					trainingRsvps
						.filter((r) => r.attended)
						.map((r) => r.userUuid)
				);
				const allNewFacils = allRsvps
					.filter((r) =>
						['facilitator', 'co-facilitator'].includes(r.type)
					)
					.map((r) => r.userUuid);
				...
				const ratios = {
					cta: {
						value: interestedFacilitators.size,
						percent:
							(100 * interestedFacilitators.size) /
							allGuests.size,
					},
					rsvpTraining: {
						value: intersect(
							allTraineeRsvps,
							interestedFacilitators
						).length,
						percent:
							(100 *
								intersect(
							...
							interestedFacilitators.size,
					},
					completeTraining: {
						value: allTraineeCompletes.size,
						percent:
							(100 * allTraineeCompletes) / allTraineeRsvps.size,
					},
					allNewFacils: {
						value: intersect(allTraineeCompletes, allNewFacils)
							.length,
						percent:
							(100 *
								intersect(allTraineeCompletes, allNewFacils)
									.length) /
							allTraineeCompletes.size,
					},
				};
