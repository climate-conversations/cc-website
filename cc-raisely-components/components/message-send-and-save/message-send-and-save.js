(RaiselyComponents, React) => {
	...
	const WhatsAppButtonRef = RaiselyComponents.import('whatsapp-button', {
		asRaw: true,
	});
	const ReturnButton = RaiselyComponents.import('return-button');
	const MessageFormatRef = RaiselyComponents.import('message-format', {
		asRaw: true,
	});
	let WhatsAppButton;
	let MessageFormat;
	...
	const initialContactFields = [
		{
			...
			default:
				"Edit the message you want to send below. A copy of this message will be automatically saved on the records of the recipients when you click send. This helps us keep good records of the contact we've had with people.",
		},
		{
			label: 'Subject',
			id: 'subject',
	...
	const userHasEmail = (user) =>
		user.email && !user.email.endsWith('.invalid');
	const getLabel = (user) =>
		user.fullName ||
		user.preferredName ||
	...
	function toggleEmailClient(contactFields, contactSettings) {
		...
		emailClient.type =
			contactSettings.sendBy === 'email' ? 'select' : 'hidden';
		return emailClient;
	}

	function Checkbox({ label, onChange, value, disabled }) {
		const labelClass = `form-field__label-text ${
			disabled ? 'disabled' : ''
		}`;
		return (
	...
	function WhatsAppLink({ recipient, onClick }) {
		if (recipient.sent || !recipient.phoneNumber) {
			return (
				<p>
					{recipient.label} {recipient.sent ? '(sent)' : ''}
				</p>
			);
		}
		return (
	...
	function RecipientList({
		...
		return (
			<div className="contact--form__recipients--whatsapp-wrapper">
				{hasWhatsapp ? (
					<p>
						Some recipients do not have an email address. Click on
						the underlined recipients below to send them a WhatsApp
					</p>
				) : (
					''
				)}
						...
						<li key={r.uuid}>
							{sendBy === 'email' && r.email ? (
								<Checkbox
									...
									onChange={(value) =>
										toggleRecipient(r.uuid, value)
									}
									value={selectedRecipients[r.uuid]}
									disabled={r.disabled || r.sent}
								/>
							) : (
								<WhatsAppLink recipient={r} onClick={onClick} />
							)}
						</li>
					))}
				</ul>
	...
	class ContactForm extends React.Component {
		...
		onContactChange = (values) => {
			...
			if (to.length > 1) {
				...
				const oldEmailClient = contactFields.find(
					(f) => f.id === 'emailClient'
				);
				const emailClient = toggleEmailClient(
					newContactFields,
					contactSettings
				);
		...
		initContactFields() {
			...
			const { sendBy, body, subject, emailClient } = Object.assign(
				contactDefaults,
				this.props
			);
			...
			return (
				<div className="contact--form__buttons">
					{isWhatsApp && recipientCount === 1 ? (
						<Button onClick={() => this.send(user, closeModal)}>
							Send WhatsApp
						</Button>
					) : (
						''
					...
					{!isWhatsApp || recipientCount === 1 ? (
						<Button
							onClick={() =>
								this.sendAll(
									recipientCount === 1 ? closeModal : null
								)
							}
							target="_blank"
						>
		...
		finalPage = () => {
			...
			const invalidRecipients = recipientFields.filter(
				(r) => !(r.phoneNumber || r.email)
			);
			return (
				<div className="contact--form">
					<div className="contact--form__final">
						{recipients.sent ? (
							<p>
								Great! {"You've"} sent messages to all valid
								recipients.
							</p>
						) : (
							''
						...
						{recipients.invalid ? (
							<React.Fragment>
								<p>
									The folllowing recipients could not be
									messaged because they have no valid contact
									details
								</p>
								<ul>
		...
		sendAll = async (closeModal) => {
			...
			const recipients = to
				...
				.filter((u) =>
					recipientFields.find(
						(f) => f.uuid === u.uuid && !f.disabled
					)
				);
			const bcc = recipients.map((u) => u.email).join(';');
			...
			const urlMap = {
				gmail: `https://mail.google.com/mail/?view=cm&fs=1&tf=1&bcc=${bcc}&su=${subject}&body=${encodeURI(
					body
				)}`,
				yahoo: `http://compose.mail.yahoo.com/?bcc=${bcc}&subject=${subject}&body=${encodeURI(
					body
				)}`,
				hotmail: `https://dub130.mail.live.com/default.aspx?rru=compose&subject=${subject}&body=${encodeURI(
					body
				)}&bcc=${bcc}#page=Compose`,
			};
		...
		send = async (user, closeModal) => {
			...
			this.saveInteraction([message]).then((canClose) => {
				user.sent = true;
		...
		saveInteraction = async (messages) => {
			...
			try {
				...
				const promises = messages.map(async (message) => {
					...
					if (type === 'whatsapp')
						data.detail.private.method = 'WhatsApp';
					if (type === 'email')
						data.detail.private.emailClient = emailClient;

					const result = await getData(
						api.interactions.create({ data })
					);
					// Uncheck the recipient
					selectedRecipients[userUuid] = false;
					const recipient = recipientFields.find(
						(r) => r.uuid === userUuid
					);
					recipient.sent = true;
					return result;
			...
			return (
				<div className="contact--form__email">
					<div className="contact--form__copy-email">
						<p>
							Copy and paste the following into an email to send
							to guests
						</p>
						<div className="contact--form__email-content">
							<div>
								...
								<span className="contact--form__value">
									{bcc}
								</span>
							</div>
							<div>
					...
					<p>
						Did you send the email? (Clicking yes will save a record
						of the email in Raisely)
					</p>
					<Button onClick={this.emailReturn}>No</Button>
					<Button
						theme="cta"
						disabled={saving}
						onClick={() => this.saveInteraction(recentlySent)}
					>
						{saving ? 'Saving' : 'Yes'}
					</Button>
				</div>
