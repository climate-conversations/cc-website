...
const CAMPAIGN_UUID = process.env.PORTAL_UUID;
...
const convertDate = (excelDate) => {
	const localDate = new tzc.DateTime(
		new Date(1900, 0, --excelDate),
		tzc.DateFunctions.Get
	);
	const sgDate = new tzc.DateTime(
		localDate.format('YYYY-MM-dd HH:mm'),
		sgTime
	);
...
const upsertPerson = cachify(
	async (user, opts = {}) => {
		...
		if (userRecord) {
			...
			if (opts.recruiter) {
				['private.recruitedBy', 'private.pointPerson'].forEach(
					(key) => {
						if (!_.get(user, key)) {
		...
		if (!user.email)
			user.email = `no-email-${_.camelCase(
				user.fullName
			)}-${unique}@noemail.invalid`;
 ...
 */
const upsertConversation = cachify(
	async (details) => {
		...
		let rsvp = allRsvps.find(
			(r) => _.get(r, 'event.private.legacyId') === details.legacyId
		);
		...
		if (existingEvent) {
			...
			if (
				eventStartAt.lessThan(details.date) ||
				eventStartAt.greaterThan(wrongTimeZone)
			) {
				console.log(
					`Event ${existingEvent.name} marked with wrong legacy id, removing`
				);
				await raiselyRequest({
		...
		if (!existingEvent) {
			const matchingRsvp = allRsvps.find((r) => {
				const eventStartAt = new tzc.DateTime(r.event.startAt);
				return (
					eventStartAt.greaterEqual(details.date) &&
					eventStartAt.lessEqual(wrongTimeZone)
				);
			});
		...
		if (existingEvent) {
			...
			if (legacyId && legacyId !== details.legacyId) {
				throw new Error(
					`Conversation has wrong legacy ID (expected none or ${details.legacyId} got ${legacyId})`
				);
			}
...
const upsertFacilitatorProfile = cachify(
	async (user) => {
...
async function upsertDonation({
	...
	const twoWeeksLater = new tzc.DateTime(conversation.startAt).add(
		2,
		tzc.TimeUnit.Week
	);
	...
	const possibleMatches = _.sortBy(donations, (d) => d.createdAt).filter(
		(d) => d.amount === cashDonation
	);
...
const upsertInteraction = cachify(
	async (interaction) => {
		...
		const existing = interactions.find(
			(i) => i.recordUuid === interaction.recordUuid
		);
...
const upsertRsvp = cachify(
	...
	(rsvp) =>
		`rsvp-${rsvp.conversation.uuid}-${rsvp.type || 'guest'}-${
			rsvp.guest.uuid
		}`
);
...
async function getHeaderRow(sheet) {
	...
	const headers = [];
	_.times(sheet.columnCount).forEach((column) => {
		...
		if ((header && headers.includes(header)) || header === 'After') {
			...
			if (lastHeader.endsWith('Before'))
				lastHeader = lastHeader.split('Before')[0].trim();
...
async function paginateRows(callback) {
	...
	while (offset <= worksheet.rowCount) {
		...
		await worksheet.loadCells({
			...
			endRowIndex: upperLimit,
		});
		for (let i = offset; i < upperLimit; i++) {
			row = {};
			header.map((title, column) => {
				row[title] = worksheet.getCell(i, column).value;
			});
...
async function syncConversations() {
	await paginateRows(async (row, offset) => {
		console.log(
			`=== (${offset}) New row ${row['Participant Name']} attended ${
				row['Host Name']
			} on ${convertDate(row['Date of Conversation']).format(
				'YYYY-MM-dd'
			)}`
		);
